			...
			<div class="row">
				<?php
					if ((isset($_SESSION['phones_to_show']) && $_SESSION['phones_to_show'] === "all_phones") || (!isset($_SESSION['phones_to_show']))) {
						...
						$sql = "SELECT * FROM phones INNER JOIN votes ON phones.id = votes.phone_id WHERE phones.type = 'feature_phone' AND votes.vote_type = 'like' AND votes.user_id = ? ORDER BY phones.make ASC";
					} elseif (isset($_SESSION['phones_to_show']) && $_SESSION['phones_to_show'] === "disliked_phones") {
						$sql = "SELECT * FROM phones INNER JOIN votes ON phones.id = votes.phone_id WHERE phones.type = 'feature_phone' AND votes.vote_type = 'dislike' AND votes.user_id = ? ORDER BY phones.make ASC";
					}
